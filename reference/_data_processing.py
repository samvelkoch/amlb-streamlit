...
def impute_results(results: pd.DataFrame, where: pd.Series,
                   with_: str = "constantpredictor",
    ...
    for index, row in results[where].iterrows():
        ...
        results.loc[index, "result"] = lookup_table.loc[
            (with_, task, fold, constraint)].result
        if indicator_column:
...
def calculate_ranks(results: pd.DataFrame) -> dict[str, float]:
    ...
    mean_performance = results[["framework", "task", "result"]].groupby(
        ["framework", "task"], as_index=False).mean()
    mean_performance["rank"] = mean_performance.groupby("task").result.rank(
